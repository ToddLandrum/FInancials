...
* Set the help file
IF FILE(lcHelpLoc + 'DMIE.chm')
    SET HELP TO (lcHelpLoc + 'DMIE.chm')
ELSE
    *  If it doesn't exist in the public documents, try the individual user's documents folder instead
    ...
    ELSE
        *  Last ditch effort - try the current folder
        IF FILE('DMIE.chm')
            SET HELP TO DMIE.chm
        ENDIF
    ENDIF
ENDIF

_SCREEN.CAPTION = 'DMIE'
...
PUBL  oMeta       && Stonefield Database Toolkit Object
...
IF NOT ISNULL(m.goApp)
    ...
    m.goApp.lAMVersion    = .F.   && Accounting Manager
    m.goApp.cSplash       = 'frmAppSplashScreen'
    m.goApp.cFileVersion  = lcVersion
    m.goApp.oMeta         = .NULL.
    m.goApp.lQBVersion    = .T.   && QuickBooks
