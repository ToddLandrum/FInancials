...
DEFINE CLASS suspense AS CUSTOM
   ...
   *******************************
   PROCEDURE Owner_suspense
      *******************************
      LPARAMETERS toProgress, tnCount

      LOCAL llReturn, loError
      *:Global cGroup
      llReturn = .T.

      SYS(3092,'temp\rushmore.txt',.T.)
      ...
      TRY
         IF m.goapp.lCanceled
            ...
            ENDIF
         ENDIF
         STRTOFILE('Owner Suspense ' + CHR(13),'temp\dispseconds.txt',1)
      ...
      lnEndSusp = SECONDS()
      STRTOFILE('End Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Owner Suspense Duration: ' + TRANSFORM(lnEndSusp-lnStartSusp) + CHR(13)+CHR(13),'temp\dispseconds.txt',1)
   ...
   PROTECTED PROCEDURE process_minimums_out
      ********************************************
      ...
      TRY
         ...
         STRTOFILE('Process Minimums Out ' + CHR(13),'temp\dispseconds.txt',1)
         STRTOFILE('Starting Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
         lnStartProc = SECONDS()
         ...
         * releasing it into invtmp
         SELECT  * ;
            ...
            WHERE cSuspType = 'M' ;
              AND cownerid NOT IN (SELECT  cownerid FROM investor WHERE lhold) ;
              AND cwellid + cownerid + cTypeInv NOT IN (SELECT cwellid + cownerid + cTypeInv FROM wellinv WHERE lonhold) ;
         ...
         INDEX ON cwellid+cownerid+cTypeInv TAG owntype
         ...
         * Get the records that match tempcurlast
         ...
         SCAN
            ...
            * Get the well history record corresponding to the owner history
            ...
            lnEnd = SECONDS()
            *            STRTOFILE('Owner: ' + m.cownerid + m.cwellid + ' Seconds: ' + TRANSFORM(lnEnd-lnStart)+CHR(13),'dispseconds.txt',1)
            lnStart = SECONDS()
         ENDSCAN
      ...
      lnEndProc = SECONDS()
      STRTOFILE('End Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Process Minimums Out Duration: ' + TRANSFORM(lnEndProc-lnStartProc) + CHR(13)+CHR(13),'temp\dispseconds.txt',1)
   ...
   PROTECTED PROCEDURE process_minimums_in
      ...
      *:Global cRunYear, ciddisb, cownerid, crunyear_in, csusptype, nRunNo, nrunno_in
      ...
      TRY
         ...
         STRTOFILE('Process Minimums In ' + CHR(13),'temp\dispseconds.txt',1)
         STRTOFILE('Starting Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
         lnStartProc = SECONDS()
         ...
         STORE 0 TO lnMinimum, lnMinCount, lnMinTot
         ...
         SCAN FOR nBalance # 0
            ...
            * We're releasing all minimums on this closing
            ...
            IF lnbalance < lnMinimum
               SELECT * FROM invtmp WHERE cownerid = m.cownerid INTO CURSOR ownermins ORDER BY cownerid
               SELECT ownermins
               SCAN
                  SCATTER MEMVAR
                  lciddisb = m.ciddisb
                  ...
                  * Swap the original run with current run
                  ...
                  IF m.nNetCheck # 0
                     lnMinCount = lnMinCount + 1
                     lnMinTot = lnMinTot + m.nNetCheck

*!*	                     * These entries could have been removed in the Process_Minimums_Out processing
*!*	                     * If they were we'll just recall them instead of adding a new record. This will
*!*	                     * save space which is important for clients like Everflow - pws 3/3/21
*!*	                     SET DELETED OFF
*!*	                     SELECT suspense
*!*	                     IF SEEK(lciddisb) AND cownerid = m.cownerid AND cwellid = m.cwellid
*!*	                        IF DELETED()
*!*	                           RECALL
*!*	                        ENDIF
*!*	                     ELSE
...
*!*	                     ENDIF
                  ...
                  ENDIF
               ENDSCAN
               DELETE FROM invtmp WHERE cownerid = m.cownerid
            ENDIF
            THIS.perflog('Minimums In - ' + m.cownerid, .T.)
            lnEnd = SECONDS()
            *            STRTOFILE('Owner: ' + m.cownerid + ' Seconds: ' + TRANSFORM(lnEnd-lnStart) + CHR(13),'temp\dispseconds.txt',1)
         ENDSCAN
      ...
      CATCH TO loError
         ...
         MESSAGEBOX('Unable to process the report at this time. Check the System Log found under Other Reports for more information.' + CHR(10) + CHR(10) + ;
            'Contact SherWare Support for help at support@sherware.com' + CHR(10) + CHR(10) + ;
            'Minimum Record Count: ' + TRANSFORM(lnMinCount) + CHR(10) + ;
            'Minimum Suspense Total: ' + TRANSFORM(lnMinTot), 16, 'Problem Encountered')
      ENDTRY

      lnEndProc = SECONDS()
      STRTOFILE('Ending Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Process Minimums In Duration: ' + TRANSFORM(lnEndProc-lnStartProc) + CHR(13)+CHR(13),'temp\dispseconds.txt',1)
      THIS.CheckCancel()
      m.goapp.Rushmore(.F.)
   ...
   PROTECTED PROCEDURE owner_holds_out
      ...
      *:Global cRunYear, cTypeInv, ciddisb, cownerid, crunyear_in, cwellid, hdate, nRunNo, nrunno_in
      ...
      TRY
         ...
         STRTOFILE('Owner Holds Out ' + CHR(13),'temp\dispseconds.txt',1)
         STRTOFILE('Starting Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
         lnStartProc = SECONDS()
         ...
         * Get the current suspense status for each owner and well.
         ...
         SELECT  cownerid, cwellid, cTypeInv ;
            ...
            GROUP BY cownerid, cwellid, cTypeInv
         INDEX ON cownerid TAG cownerid
         INDEX ON cownerid+cwellid+cTypeInv TAG ownertype

         * Get the suspense records for the owners that are in deficit into a cursor
         SELECT suspense.* FROM suspense ;
            WHERE cownerid+cwellid+cTypeInv IN (SELECT cownerid+cwellid+cTypeInv FROM tempcurlast) ;
            INTO CURSOR curOwnerHolds ;
            ORDER BY cownerid

         SELECT cownerid FROM tempcurlast INTO CURSOR heldowners ORDER BY cownerid GROUP BY cownerid
         ...
         SCAN
            m.cownerid = cownerid
            IF THIS.at_pay_freq(m.cownerid)
               ...
               SCAN FOR cownerid = m.cownerid
                  ...
                  SCAN FOR cownerid = m.cownerid ;
                     ...
                     * Store the original run into run_in
                     ...
                     IF NOT SEEK(m.cWellID + m.cOwnerid + m.cTypeInv)
                        loop
                     ENDIF
                     ...
                     * Reset the on hold flag in the record
                     ...
                     ENDIF
                     INSERT INTO invtmp FROM MEMVAR
                     INSERT INTO delsusp VALUES (lciddisb)
                     IF NOT THIS.lClosing
                        * Get the well history record corresponding to the owner history
                        THIS.getwellhist(m.cwellid, m.hyear, m.hperiod)
                     ENDIF
                  ENDSCAN && curOwnerHolds
               ENDSCAN && tempcurlast
            ENDIF && atpayfreq
         ENDSCAN && heldowners
      ...
      lnEndProc = SECONDS()
      STRTOFILE('Owner Holds Out Ending Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Owner Holds Out Duration: ' + TRANSFORM(lnEndProc-lnStartProc) + CHR(13)+CHR(13),'temp\dispseconds.txt',1)
   ...
   *****************************************
   PROTECTED PROCEDURE owner_deficits_in
      ...
      *  Place them in suspense
      ...
      STRTOFILE('Owner Deficits In ' + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Starting Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      lnStartProc = SECONDS()
      ...
      TRY
         ...
         * Create a cursor to store the keys of deleted invtmp records
         CREATE CURSOR delhist (ciddisb c(8))
         ...
         * Scan through the unique owner combinations so we can get the current balance for the given well
            ...
            *  Get what their current suspense balance is, and add it to the current balance.  If that total is negative, add it to suspense.
            ...
            SCAN FOR cownerid == m.cownerid
               ...
               IF m.nNetCheck # 0
                  m.ciddisb = THIS.get_next_key('disbhist')
                  INSERT INTO suspense FROM MEMVAR
                  INSERT INTO delhist FROM MEMVAR
                  REPLACE ciddisb WITH m.ciddisb IN invtmp
                  IF !THIS.lNewRun AND m.nrunno_in = THIS.nRunNo AND m.crunyear_in = THIS.cRunYear
                     INSERT INTO tsuspense FROM MEMVAR
                  ENDIF
               ELSE
         ...
         IF RECCOUNT('delhist') > 0
            DELETE FROM invtmp WHERE ciddisb IN (SELECT ciddisb FROM delhist)
         ENDIF
      ...
      lnEndProc = SECONDS()
      STRTOFILE('Ending Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Owner Deficits In Duration: ' + TRANSFORM(lnEndProc-lnStartProc) + CHR(13)+CHR(13),'temp\dispseconds.txt',1)
   ...
   ***************************************
   PROCEDURE owner_suspense_balance
      ***************************************
      ...
      m.goapp.oLogger.LogMileStone('Suspense: Owner_Suspense_Balance - Start')
      ...
      TRY
         ...
         STRTOFILE('Owner Suspense Balance ' + CHR(13),'temp\dispseconds.txt',1)
         STRTOFILE('Starting Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
         lnStartProc = SECONDS()
         ...
         THIS.perflog('Suspense: Owner Suspense Balance')
         THIS.timekeeper('Owner Suspense Balance Start')
         m.lcRunYear = THIS.cRunYear + PADL(TRANSFORM(THIS.nRunNo), 3, '0')
               ...
               * Calculates owner's suspense balance overall before this run
               IF tlIgnoreHoldStatus
                  SELECT  suspense.cownerid, ;
                     ...
                     SUM(suspense.nNetCheck) AS nBalance ;
                     FROM suspense  WITH (BUFFERING = .T.)  ;
                     WHERE suspense.cGroup   == m.tcGroup ;
                     AND (crunyear_in + PADL(TRANSFORM(nrunno_in), 3, '0') < m.lcRunYear) ;
                     AND (suspense.cprogcode = ' ' ;
                     OR suspense.cprogcode IN (SELECT  cprogcode FROM programs WHERE lprognet = .T.));
      ...
      lnEndProc = SECONDS()
      STRTOFILE('Ending Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Duration: ' + TRANSFORM(lnEndProc-lnStartProc) + CHR(13),'temp\dispseconds.txt',1)
      m.goapp.oLogger.LogMileStone('Suspense: Owner_Suspense_Balance - End')
         ...
         ELSE
            IF THIS.at_pay_freq(tcOwnerid)
               SELECT  cwellid ;
                  ...
                  INTO CURSOR tempwells NOFILTER ;
               ...
               IF NOT tlIgnoreProgs
                  * Calculates owner's suspense balance overall before this run
                  SELECT  SUM(nNetCheck) AS nBalance ;
                     ...
                     FROM tempwells) ;
                     INTO CURSOR tempbalance1 NOFILTER
                  ...
                  SELECT  SUM(nNetCheck) AS nBalance ;
                     ...
                     FROM tempwells) ;
                     INTO CURSOR tempbalance2 NOFILTER
                  ...
                  IF THIS.lNewRun
                     SELECT  SUM(nNetCheck) AS nBalance ;
                        ...
                        FROM tempwells) ;
                        AND IIF(NOT tlIncludePayments, crectype # 'P', .T.) ;
                        INTO CURSOR tempbalance3 NOFILTER
                     lnTally = lnTally + _TALLY
                  ENDIF
               ELSE
                  * Calculates owner's suspense balance overall before this run
         ...
         IF NOT tlAfterRun
            ...
            * Get the last suspense entry for each well
            ...
            SELECT  disbhist.cwellid, ;
               ...
               AND IIF(NOT tlIncludePayments, disbhist.crectype # 'P', .T.) ;
   ...
   ***************************************
   FUNCTION check_for_payments
      ***************************************
      ...
      IF FILE(m.goapp.cCommonFolder+'nochkpmts.txt')
         RETURN .T.
      ENDIF
      ...
      STRTOFILE('Calling from Check_For_Payments','temp\dispmemo.txt',1)
      STRTOFILE('Calling from Check_For_Payments','temp\dispstatus.txt',1)
      ...
      TRY
         ...
         STRTOFILE('Check for Pmts ' + CHR(13),'temp\dispseconds.txt',1)
         STRTOFILE('Starting Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
         lnStartProc = SECONDS()
         ...
         * Get the global minimum check amount
         ...
         THIS.oProgress.SetProgressMessage('Processing Suspense by Owner... Check for payments - Setup')
         THIS.oProgress.UpdateProgress(THIS.nProgress)
         THIS.nProgress = THIS.nProgress + 1
      ...
      lnEndProc = SECONDS()
      STRTOFILE('End Time: ' + TTOC(DATETIME()) + CHR(13),'temp\dispseconds.txt',1)
      STRTOFILE('Check for Pmts Duration: ' + TRANSFORM(lnEndProc-lnStartProc) + CHR(13),'temp\dispseconds.txt',1)
